  ...
  useEffect(() => {
    async function fetchData() {
        try {
            ...
            let recent = null;
            if (data.tipo === 'DOADOR' && data.doador?.coletas?.length > 0) recent = data.doador.coletas[0]; 
            else if (data.tipo === 'VOLUNTARIO' && data.voluntario?.coletasRealizadas?.length > 0) recent = data.voluntario.coletasRealizadas[0];
            else if (data.tipo === 'EMPRESA' && data.empresa?.coletasRecebidas?.length > 0) recent = data.empresa.coletasRecebidas[0];
  ...
  return (
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border self-start md:self-auto ${
      ...
      <div>
        ...
        {recentColeta ? (
           ...
           <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 border-l-4 border-green-500 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:shadow-md transition-shadow">
              ...
              <button 
                ...
                className="w-full md:w-auto px-5 py-3 md:py-2.5 bg-gray-50 text-gray-700 hover:text-green-700 hover:bg-green-50 font-semibold rounded-lg transition-all text-sm md:text-xs border border-gray-200 cursor-pointer text-center"
              >
           ...
           <div className="bg-gray-50 p-8 rounded-xl border-2 border-dashed border-gray-200 text-center">
              <p className="text-gray-400 font-medium mb-3 text-sm">Nenhuma atividade recente.</p>
              <button 
                 onClick={() => router.push('/dashboard/coletas')}
                 className="text-green-600 font-bold text-xs hover:underline cursor-pointer bg-transparent border-none"
              >
         ...
         {campanhasDisponiveis.length > 0 ? (
             ...
             <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
                 {campanhasDisponiveis.slice(0, 2).map((camp: any) => (
                     <div key={camp.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                         <h3 className="font-bold text-base text-gray-800">{camp.titulo}</h3>
             ...
             <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-center min-h-[120px]">
